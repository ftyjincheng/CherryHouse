...
<view class="container">
	<swiper @tap="previewImage" :swiperImg="images"></swiper>
	<view class="top">
		<view class="t2">
			<view>
	...
	<view class="content">
		...
		<image @tap="deal" class="pc" :src="imgs" mode="aspectFill"></image>
		<view class="s">
			<text>房主说:</text>
			<text>{{msg}}</text>
		</view>
		<view class="sd">
			<image :class="advertisingImage==''?'hidden':''" @tap="other" :src="advertisingImage[0]" mode="aspectFill"></image>
		</view>
		<view class="sd">
			<image :class="advertisingImage==''?'hidden':''" @tap="other" :src="advertisingImage1[0]" mode="aspectFill"></image>
		</view>
		<view class="side ">位置及周边</view>
		<map :longitude="longitude" :latitude="latitude" :markers="markers"></map>
		<view @tap="goodHouse" class="side f">
			<text>优选房源</text>
			<view class="sideL">
				<text>查看全部</text>
				<text class="icon-you"></text>
		...
		<view @tap="personal" class="side f">
			<text>房东直卖</text>
			<view class="sideL">
				<text>查看全部</text>
				<text class="icon-you"></text>
		...
		<view @tap="newHouse" class="side f">
			<text>周边新房</text>
			<view class="sideL">
				<text>查看全部</text>
				<text class="icon-you"></text>
	...
	<view class="footer2 ">
		<view class="n">
			<image src="/static/assets/home/Headportrait/s01.png" mode="aspectFill"></image>
			<view class="n1">
				<text>{{info.name}}</text>
				<text>{{info.type=='0'?"房主":"经纪人"}}</text>
			</view>
		</view>
		<button @tap="deal" class="btn2">交易服务</button>
		<button class="btn3" @getuserinfo="getuserinfo" open-type="getUserInfo">联系房主</button>
	</view>
...
export default {
  data() {
    return {
      imgs: "/static/assets/home/butie.png",
      ...
      // 轮播图
      images: ["https://ss3.bdstatic.com/70cFv8Sh_Q1YnxGkpoWK1HF6hhy/it/u=4018216757,316269704&fm=26&gp=0.jpg", "https://ss3.bdstatic.com/70cFv8Sh_Q1YnxGkpoWK1HF6hhy/it/u=4018216757,316269704&fm=26&gp=0.jpg", "https://ss3.bdstatic.com/70cFv8Sh_Q1YnxGkpoWK1HF6hhy/it/u=4018216757,316269704&fm=26&gp=0.jpg", "https://ss3.bdstatic.com/70cFv8Sh_Q1YnxGkpoWK1HF6hhy/it/u=4018216757,316269704&fm=26&gp=0.jpg"],
      // 活动图
  ...
  onLoad(options) {
    console.log(options); // 传递参数动态修改标题
                ...
                uni.request({
                  ...
                  success(res) {
                    ...
                    uni.request({
                      ...
                      success(res) {
                        //换取toen
                        console.log(res); //获取用户信息  0为黑名单

                        let blacklist = res.blacklist;
                        uni.setStorageSync("blacklist"); // 0为黑名单  1为正常
